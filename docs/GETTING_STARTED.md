...
This guide uses the published Maven Central release **`1.3.2-alpha03`**.

## 1. Requirements

SKOne `1.3.2-alpha03` is built and tested with the toolchain below (from this repository’s Gradle configuration):
...
`skone-bom` pins compatible versions of all eight SKOne library modules. Declare the BOM once, then add module dependencies **without** repeating version numbers. Gradle resolves matching `1.3.2-alpha03` artifacts for every SKOne module on your classpath.
...
In **`1.3.2-alpha03`**, the Compose `SKTextField` uses an integrated **floating-label outlined field**:
...
This improved floating-label UX applies to the **Compose** `SKTextField` in `1.3.2-alpha03`. The XML `SKTextFieldView` uses a separate layout and does not share this Compose implementation.
...
| Sample | Purpose |
|--------|---------|
| [`samples/skone-demo/`](../samples/skone-demo/) | External Maven Central consumer (`1.3.2-alpha03`) — no `project(...)` dependencies |
